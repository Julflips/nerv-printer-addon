...
public class CarpetPrinter extends Module implements MapPrinter {
    ...
    Pair<Integer, Integer> trueInterval; // Stores the actual interval in case the old one is temporarily overwritten
                                         // while repairing
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (state == State.Dumping) {
            ...
            if (dumpSlot == -1) {
                ...
                Pair<ArrayList<Integer>, HashMap<Item, Integer>> invInformation = Utils.getInvInformation(requiredItems,
                        availableSlots);
    ...
    private void interactWithBlock(BlockPos blockPos) {
        ...
        BlockHitResult hitResult = new BlockHitResult(blockPos.toCenterPos(), Utils.getInteractionSide(blockPos),
                blockPos, false);
    ...
    private void calculateBuildingPath(boolean cornerSide, boolean sprintFirst) {
        ...
        for (int x = workingInterval.getLeft(); x <= workingInterval.getRight(); x += linesPerRun.get()) {
            ...
            for (int lineBonus = 0; lineBonus < linesPerRun.get(); lineBonus++) {
                ...
                if (adjustedX > workingInterval.getRight())
                    break;
                for (int z = 0; z < 128; z++) {
